## Table of Contents
Modules
	[[MPU6050 and Hardware Interface Documentation#`HardwareInterface`|Hardware Interface]]
	[[PID Controller and Movement Package Documentation#`Movement Package`|Movement Package]]
	[[Camera Package#`Camera Package`|Camera Package]]
...
flowchart LR
	...
	controller_on_Surface --> inputs_db
	inputs_db --> Movement_Package
	Movement_Package --> outputs_db 
	outputs_db --> Hardware_Interface
	Sonar_Module --> sonar_db
	Hardware_Interface --> sensors_db
	Camera_Package --> Neural_Network
	Neural_Network --> State_Machine
	Neural_Network --> objects_db
	State_Machine --> inputs_db
	sonar_db --> State_Machine
	Camera_Package --> main
```
# Project Documentation
## Overview
This project consists of a backend system handling multiple subprocesses:
- Hardware Interface
- Movement Package
- Neural Network Package
- Web Interface
These subprocesses are run in parallel and communicate via a custom database system. The main script handles the communication between these subprocesses and manages the state machine, which controls the overall project.
## Main Components
### Flask App
The backend is built using Flask, a micro web framework for Python. The Flask app handles various routes and connects to an SQLite database.
### Database Models
Four main database models are used to store data:
- `Sensors`: Stores sensor data such as voltages, currents, depth, orientation, temperature, humidity, and heading.
- `Output`: Stores output data for motors, claw, and torpedoes.
- `Input`: Stores input data for movement, orientation, claw, and torpedoes.
- `Objects`: Stores detected objects with their distance and angle.
### Subprocesses
Subprocesses are launched using the `subprocess` module. The subprocesses include:
- Hardware Interface
- Movement Package
- Neural Network
- Camera Package
...
class Sensors(db.Model):
    ...
    def __repr__(self):
        return f'<{self.id}, {self.voltage1}, {self.voltage2}, {self.volage3}, {self.current1}, {self.current2}, {self.current3}, {self.depth}, {self.X}, {self.Y}, {self.Z}, {self.pitch}, {self.roll}, {self.yaw}, {self.temperature}, {self.orin_temp}, {self.humidity}, {self.heading}>'
...
class Output(db.Model):
    ...
    def __repr__(self):
        return f'<{self.id}, {self.M1}, {self.M2}, {self.M3}, {self.M4}, {self.M5}, {self.M6}, {self.M7}, {self.M8}, {self.Claw}, {self.Torp1}, {self.Torp2}>'
...
class Input(db.Model):
    ...
    def __repr__(self):
        return f'<{self.id}, {self.X}, {self.Y}, {self.Z}, {self.pitch}, {self.roll}, {self.yaw}, {self.claw}, {self.torp1}, {self.torp2}>'
