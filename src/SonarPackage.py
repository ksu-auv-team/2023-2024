...
class Sonar:
    def __init__(self):
        self.p = Ping360()
        self.p.connect_serial("/dev/ttyUSB0", 115200) ##Connects to sonar
        self.p.initialize()##Initializes
        self.p.set_transmit_frequency(750)##Sets frequency of tranmission, limited to 650 to 850 for practicallity
        self.p.set_sample_period(1355)##Sets sample period (increase to increase range) (range is 80 to 40000)
        self.p.set_number_of_samples(1200)##Sets number of samples (increase to increase range). (range is 200 to 1200)
        ...
        while True:
            try:
                for gradian in range(400):
                    highestIndex = 0
                    highestValue = 0
                    d = self.p.transmitAngle(gradian) ##Fires pulse in a gradian direction
                    distperSample = self.meters_per_sample(d, 1480) ##Determines the distance of a single sample
